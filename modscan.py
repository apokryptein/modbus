...
def main():
	...
	# BT: changed default timeout to 20ms to speed up scanning -- it was running really slowly
	p.add_option('--timeout', '-t', type='int', dest="timeout", default=20, help='socket timeout (mills) DEFAULT:20')
	p.add_option('--aggressive', '-a', action ='store_true', help='continues checking past first found SID')
	# BT: changed default to function 1, as prior default function 17 is for serial only
	# BT: this will allow for a more globally useful program
	p.add_option('--function', '-f', type='int', dest="function", default=1, help='MODBUS Function Code DEFAULT:1')
	#BT: Added a default data type to match modbus function 1 for it to be functional
	p.add_option('--data', type='string', dest="fdata", default="\x00\x00\x00\x01", help='MODBUS Function Data.  Unicode escaped "\x00\x00\x00\x01"')
	...
	if len(arguments) == 1:
		...
		for ip in iprange:
			...
			# BT: Changed range to include 247
			for sid in range (1, 248):
				...
				try:
					...
					# BT: added conditional as error was tripped during each usage of program
					if (sid == 247):
						break
					else:
						fError=1
